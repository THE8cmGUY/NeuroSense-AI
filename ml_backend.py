...
# ═══════════════════════════════════════════════════
# firebase-keys.json kaise milegi:
# Firebase Console → Project Settings →
# Service Accounts → Generate New Private Key
# → File download karke project folder mein rakho
# ═══════════════════════════════════════════════════
cred = credentials.Certificate("firebase-keys.json")
...
while True:
    try:
        ...
        if historical_data and len(historical_data) > 10:
            ...
            # FIX 1: 'temp' use karo (ESP32 yahi key use karta hai)
            temp_list = [v['temp'] for k, v in historical_data.items() if 'temp' in v]
            ...
            print(f"[AI KERNEL] Training LSTM on {len(df)} records...")
            model.fit(X, y, epochs=10, batch_size=2, verbose=0)
            print("[AI KERNEL] Training Complete.")

            live_data = db.reference('incubator').get()

            if live_data:
                # FIX 2: Har bay ke liye loop — dynamic, koi hardcoding nahi
                for bay_id, bay_data in live_data.items():
                    if not isinstance(bay_data, dict) or 'temp' not in bay_data:
                        continue
                    ...
                    # FIX 3: Har bay ka apna path
                    db.reference(f'incubator/{bay_id}/ml_prediction').set({
                        'future_temp': future_temp,
                        'ml_alert':    ml_warning
                    })
                    print(f"[CLOUD] {bay_id} → {future_temp}°C | {ml_warning}")
            else:
                print("[WARNING] No live incubator data in Firebase.")
        else:
            print("[WARNING] Need minimum 10 historical records for LSTM.")
